...
{  
    ...
    *length = 0;
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        i++;
        file_content = realloc(file_content, sizeof(char)*(i+1));
    }
...
const char* lookup(const char* path)
{
    ...
    if (strcasecmp(point, ".css") == 0) return "text/css";
    
    else if(strcasecmp(point, ".html")== 0) return "text/html";
    
    else if(strcasecmp(point, ".gif") == 0) return "image/gif";
    
    else if(strcasecmp(point, ".ico") == 0) return "image/x-icon";
    
    else if(strcasecmp(point, ".jpg") == 0) return "image/jpeg";
    
    else if(strcasecmp(point, ".js") == 0) return "text/javascript";
    
    else if(strcasecmp(point, ".php") == 0) return "text/x-php";
    
    else if(strcasecmp(point, ".png") == 0) return "image/png";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char request_target[strlen(line) + 1];
    char httt_version[strlen(line) + 1];
    char tail[strlen(line) + 1]; 
    
    //devide line into string, "space" - devider
    int n = sscanf ( line,"%s %s %s %s",method,request_target,httt_version,tail);
    
     
     //check if line consist more than 2 spaces in a row or 4 spaces
     if ((n!=3)||(strstr(line, "  ")))
   {
       error(400);
       return false;
    ...
    if(strcasecmp(httt_version,"HTTP/1.1" )!= 0){
        error(505);
        return false;  
    }
    
    //если запрос не содержит ?, возварщаем путь и пустой запрос
    if(strchr(request_target,'?') == NULL){
        strcpy(abs_path, request_target);
        strcpy(query, "");
    }
    else{
        //вычисляем длину искомой строки с помощью разницы двух указателй на эту строку
        strncpy(abs_path, request_target, strchr(request_target, '?') - request_target);
